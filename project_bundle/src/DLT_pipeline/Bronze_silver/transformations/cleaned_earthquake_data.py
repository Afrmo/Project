...
def earthquake_data():
    ...
    df = df.select('features.properties.*',"features.id", col("features.geometry.coordinates")[0].alias("longitude"),
                  col("features.geometry.coordinates")[1].alias("latitude"),col("features.geometry.coordinates")[2].alias("depth"),"_load_timestamp")
    df = (
...
dlt.create_streaming_table(name='earthwuake_data_final')

dlt.apply_changes(
    target = 'earthwuake_data_final',
